...
import { PLANS, DISCOUNTED_PRICES, getLoyaltyLevel } from '../config/pricing';
import { hapticFeedback } from '../api/client';
...
export function Shop() {
  ...
  const [selectedPayment, setSelectedPayment] = useState<'stars' | 'sbp' | 'card'>('stars');
  const [promoCode, setPromoCode] = useState('');
  
  // TODO: Get from API
  const currentPoints = 320;
  const levelInfo = getLoyaltyLevel(currentPoints);
  const currentDiscount = levelInfo.discount;
  const levelKey = levelInfo.name.toLowerCase() as keyof typeof DISCOUNTED_PRICES;

  const selected = PLANS.find(p => p.id === selectedPlan);
  const finalPrice = selected 
    ? DISCOUNTED_PRICES[levelKey]?.[selected.id as keyof typeof DISCOUNTED_PRICES.bronze] || selected.basePrice
    : 0;
  ...
  const handlePaymentSelect = (methodId: 'stars' | 'sbp' | 'card') => {
    hapticFeedback('light');
  ...
  const handlePurchase = () => {
    ...
    console.log('Purchase:', { selectedPlan, isGift, selectedPayment, promoCode });
  };
    ...
    <div className="max-w-md mx-auto px-4 pt-6 pb-6 space-y-6">
      ...
      <div className="space-y-3">
        {PLANS.map((plan) => {
          ...
          const savings = plan.months > 1 
            ? Math.round((1 - (discountedPrice / plan.months) / DISCOUNTED_PRICES[levelKey]['1m']) * 100)
            : 0;
        ...
        <div className="space-y-2">
          {paymentMethods.map((method) => (
            ...
            >
              ...
              {method.id === 'stars' && (
                <span className="text-xs text-[#6B7280]">~{Math.round(finalPrice * 1.5)} ⭐</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="rounded-2xl bg-gradient-to-br from-[#1A1A1A] to-[#2A2A2A] p-6 border border-white/10 space-y-3">
        <div className="flex justify-between text-sm">
          ...
          <span className={currentDiscount > 0 ? 'line-through' : ''}>{selected?.basePrice} ₽</span>
        </div>
        {currentDiscount > 0 && (
          <div className="flex justify-between text-sm">
            <span className="text-[#6B7280]">Скидка {levelInfo.nameRu} ({currentDiscount}%)</span>
            <span className="text-[#10B981]">
              −{selected ? selected.basePrice - finalPrice : 0} ₽
            </span>
          </div>
        )}
      ...
      <button 
        ...
        className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] font-semibold text-lg hover:shadow-lg hover:shadow-[#6366F1]/50 transition-shadow flex items-center justify-center gap-2 active:scale-[0.98]"
      >

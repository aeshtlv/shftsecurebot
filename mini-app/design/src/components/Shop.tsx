...
export function Shop() {
  ...
  const currentDiscount = 5; // From loyalty level
  const selected = plans.find(p => p.id === selectedPlan);
  const finalPrice = selected ? Math.round(selected.price * (1 - currentDiscount / 100)) : 0;
    ...
    <div className="max-w-md mx-auto px-4 pt-6 pb-6 space-y-6">
      ...
      <div className="space-y-3">
        {plans.map((plan) => {
          ...
          const discountedPrice = Math.round(plan.price * (1 - currentDiscount / 100));
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-[#6B7280] mt-1">
                    {Math.round(plan.pricePerMonth * (1 - currentDiscount / 100))} ₽/месяц
                  </p>
                </div>
              </div>
            </button>
        ...
        <div className="space-y-2">
          {paymentMethods.map((method) => (
            ...
            >
              ...
              {method.id === 'stars' && (
                <span className="text-xs text-[#6B7280]">~{Math.round(finalPrice / 2)} ⭐</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="rounded-2xl bg-gradient-to-br from-[#1A1A1A] to-[#2A2A2A] p-6 border border-white/10 space-y-3">
        ...
        <div className="flex justify-between text-sm">
          <span className="text-[#6B7280]">Скидка лояльности ({currentDiscount}%)</span>
          <span className="text-[#10B981]">
            −{selected ? selected.price - finalPrice : 0} ₽
          </span>
        </div>
      ...
      {/* Purchase Button */}
      <button className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] font-semibold text-lg hover:shadow-lg hover:shadow-[#6366F1]/50 transition-shadow flex items-center justify-center gap-2">
